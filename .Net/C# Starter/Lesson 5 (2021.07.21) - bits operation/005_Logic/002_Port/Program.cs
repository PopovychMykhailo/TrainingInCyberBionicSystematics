...
namespace Logic
{
    class Program
    {
        static void Main()
        {
            ...
            Console.WriteLine($"port = {port:X}"); // ситуация с лампочкой в комнате

            port = (byte)(port | mask);  // Включить устройство управляемое первым битом.
            Console.WriteLine($"port = {port:X}");

            mask = 0xFD;          // 1 1 1 1   1 1 0 1  -  Создаем маску выключения устройства управляемого первым битом.       // 1 1 1 1   0 0 1 0
