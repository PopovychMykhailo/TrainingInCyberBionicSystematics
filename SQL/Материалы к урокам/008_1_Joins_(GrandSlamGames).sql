...
--CREATE TABLES
CREATE TABLE dbo.Players	--описание игрока
(
...
CREATE TABLE dbo.PlayerInfos	-- дополнительная информация об игроке
(
    Id int NOT NULL IDENTITY PRIMARY KEY,
...
-- (объеденить таблицы Players и PlayerInfos)
SELECT
	p.Rank,
...
-- Вывести информацию о кортах: 
---- какие матчи проводились на каждом корте, какого типа (финал, полуфинал, ...),
---- дату и время, во время какого турнира
-- (объеденить таблицы Courts, Matches и Tournaments)
...
-- Вывести статистику содержащую следующую информацию:
---- название корта где проводилась игра, дата и время игры, имя победителя, 
---- имя проигравшего, aces победителя и проигравшего
-- Для этого нужно объеденить информацию из таблиц: Matches, PlayerStats, Players, Courts 
...
--вариант 2 с использованием SELF JOIN:
SELECT 
	c.Name,
	m1.Date,
	m1.Time,
...
-- Вывести информацию о том против против кого играли победители своих матчей
-- Для этого нужно объеденить информацию из таблиц: 
---- PlayerStats и Players + использовать SELF JOIN
...
-- FULL JOIN
-- Вывести информацию  о матчах: 
---- номер матча, имя турнира в контексте которого он происходил, 
---- тип матча (финал, полуфинал, ...), дату и время матча, 
---- полную информацию о корте где происходил матч
-- Дополнительно в выборке должна присутствовать информация о матчах корт для которых 
-- ещё небыл выбран, а также информация о кортах на которых не было ниодного матча
...
FROM Matches m
	FULL JOIN Courts c 
		ON c.Id = m.CourtId
	FULL JOIN Tournaments t
		ON m.TournamentId = t.Id
GO

--CROSS JOIN (используются подзапросы)
-- Вывести статистику сколько матчей выиграл игрок на каждом из кортов
...
-- Вывести статистику сколько матчей выиграл игрок на каждом из турниров
